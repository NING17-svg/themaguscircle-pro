# The Magus Circle Reference Site

Generated game-guide site for **The Magus Circle** at `themaguscircle.pro`.
Built on the V3 Next.js seed template by the `one-click-builder` role. The
neutral theme and empty asset manifest are development fallbacks; the
production theme and content packages are applied by the configurator and
theme-implementer skills after this baseline.

See `AGENTS.md`, `CONTENT_INDEX.md`, and `GROWTH_LOG.md` for the operating
contract, page inventory, and change history of the live site.

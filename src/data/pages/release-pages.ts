...
export const page_release_date: PageContent = {
  ...
  modules: [
  {
    ...
    "body": "The Magus Circle released on September 24, 2026 on Steam (AppID 2796640), developed and published by Witchlight. The Steam store page now lists the title as released with 179 reviews carrying a Very Positive (92%) label. Witchlight shipped Patches 1.0.1, 1.0.2, and the 1.0.3 hotfix in quick succession; the Steam Community Hub lists the 1.0.3 patch notes above the prior two updates. After 1.0.3 the only two reported bugs still open are the post-Matcha character-selection reset and the Wacom Cintiq cursor flicker. See the What changed in 1.0.3 and Known issues sections below before installing.\n\n## What the Steam store page lists now that The Magus Circle has launched\n\nThe Magus Circle is listed on the Steam store page (AppID 2796640) as released on September 24, 2026. Witchlight is credited as both developer and publisher. The store page shows one base listing rather than separate regional SKUs, and the supported language set remains English and Korean for interface, full audio, and subtitles.\n\nCurrent launch facts as of 2026-09-27:\n\n- Release: September 24, 2026 on Steam (AppID 2796640)\n- Platform at launch: Windows PC (Steam)\n- Developer and publisher: Witchlight\n- Steam user reviews: 179 reviews, Very Positive (92%)\n- Interface and full audio and subtitles: English, Korean\n- Current build: Patch 1.0.3 hotfix (Sep 25, 2026)\n\nThe SteamDB record for AppID 2796640 mirrors the release status and is used here as a cross-check, not as a separate timing source. The review count and Very Positive label are read directly from the store page and reflect the cumulative user-review state of the listing.\n\n## Day 1 patches 1.0.1 and 1.0.2, then Patch 1.0.3 hotfix\n\nWitchlight is actively patching the launch build. Three updates have shipped by name in the Steam Community Hub and in launch coverage:\n\n- **Patch 1.0.1 (Day 1 stability)** — landed alongside or just after release. This patch focuses on launch-day stability, crash fixes, and the first wave of post-launch regressions reported by players in the Steam Community Hub.\n- **Patch 1.0.2 (hotfix)** — the first hotfix layered on top of 1.0.1. This patch addresses follow-up bugs that surfaced once 1.0.1 reached a wider audience, including character-specific fix scopes for Wheel, Hermit, Matcha, and Anchan.\n- **Patch 1.0.3 (hotfix, Sep 25)** — the current build. Witchlight's \"1.0.3 Hotfix!\" event on the Steam Community Hub explicitly enumerates the closed bug list (see the callout below).\n\nA Witchlight launch statement recorded by Happy Gamer also commits to \"continue to provide support and updates for the next couple months,\" so the patch cadence is expected to continue beyond 1.0.3. The exact release timestamps for each patch should be re-checked on the Steam news hub; this page records the patch names and scopes, not the precise minute each one shipped.\n\n## What changed in Patch 1.0.3\n\nThe Patch 1.0.3 post on the Steam Community Hub lists every bug this hotfix closes and the one balance change worth flagging. Treat this as the authoritative closure set rather than the residual list below.\n\n- **Bugs fixed:** Exsanguinate getting stuck without exploding, accidental extractions, Riptide, Poppet Master upgrade, ending sequence failing to reset, spell effects getting stuck on screen, chest pickup logic, Jam pausing with the game, Milk no longer forced until Matcha unlock, incorrect reroll count, incorrect ability charge count, Mana Efficiency display, missing Steam achievement, achievements unlocking too soon, and potion craft tutorial fixes.\n- **Balance change:** No more star-shaped shielded enemies. Exsanguinate area reduced by 25%; Ink base area increased slightly with Inky Area scaling decreased; Prismatic Gale no longer costs mana.\n- **Enhancement:** New spell scrolls are acquirable by exploring the hub.\n\nIf you were holding off because of any item in the fixed list, 1.0.3 is the build to install. The two residual bugs below are the only items the page still tracks as open.\n\n## Known issues still open after Patch 1.0.3\n\nTwo reported bugs remain visible in the Steam Community Hub after the 1.0.3 hotfix and are not listed as fixed in any shipped patch. They are recorded here so readers do not assume the current build is bug-free.\n\n- **Character-selection reset (post-Matcha unlock only)** — until Patch 1.0.3 the launcher forced Milk on every new save; 1.0.3 closes that pre-Matcha lock. The active Steam Community Hub thread \"Can't change characters\" still tracks a residual reset for players who have already unlocked Matcha. Affected characters named in the thread include Macha (Matcha), Coffee, and the flower-girl unlockable.\n- **Wacom Cintiq cursor flicker** — stylus / drawing-tablet users on Wacom Cintiq hardware can see the cursor flicker while tracing sigils. This is a separate issue from the character-selection bug and is not mentioned in the 1.0.1, 1.0.2, or 1.0.3 fix scopes.\n\nIf a reader hits either bug, the right move is to subscribe to the Steam news hub so a future patch (1.0.4 or later) lands in the email, and to follow the Steam Community Hub threads where Witchlight and the community track workarounds. The Known issues section will be updated once a future patch closes either entry.\n\n## How to follow official release news going forward\n\nDate changes after launch are unlikely; what changes now is the patch cadence. Two channels carry the next round of updates first: the Steam news hub on the store page and Witchlight's own Steam posts. Treat third-party trackers as echoes of those sources, not as primary timing.\n\nFor related status checks in the days after launch:\n\n- [Demo status](/demo) — confirm whether a separate demo app appears.\n- [Price](/price) — confirm the published retail figure and the introductory-offer end date.\n- [Platforms](/platforms) — confirm whether Windows remains the only platform.\n- [Download](/download) — prepare the Steam install path or fix a broken install.\n- [System requirements](/system-requirements) — confirm Windows minimum specs before installing.\n- [Characters](/characters) — see the named playable mage roster and the 1.0.3 Poppet Master / Matcha-unlock fixes."
  },
  {
    "id": "sources",
    "type": "prose",
    ...
    "links": [
      {
        ...
        "description": "`official/store` - checked `2026-09-27` - released Sep 24, 2026; 179 reviews, Very Positive (92%); Witchlight developer and publisher; English and Korean language support."
      },
      {
        "label": "Steam Community Hub — The Magus Circle (AppID 2796640)",
        "href": "https://steamcommunity.com/app/2796640",
        "description": "`community` - checked `2026-09-27` - '1.0.3 Hotfix!' event posted Sep 25 with the closed bug list, patch names (1.0.1, 1.0.2, 1.0.3), named mage roster, and the active 'Can't change characters' thread tracking the residual post-Matcha reset."
      },
      {
        ...
        "description": "`press` - checked `2026-09-27` - Day 1 stability framing, drawing-tablet support coverage, and Witchlight's continued-support commitment."
      },
      {
        "label": "steam250 club entry — AppID 2796640",
        "href": "https://club.steam250.com/app/2796640",
        "description": "`reference` - checked `2026-09-27` - review-count and review-label cross-check against the Steam store page."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "body": "- Status as of 2026-09-27. The Magus Circle released on September 24, 2026; the 179-review Very Positive (92%) label and the Patch 1.0.1 / 1.0.2 / 1.0.3 names are read from the Steam store page and the Steam Community Hub.\n- Patch names and scopes are recorded here from the Steam Community Hub threads and launch coverage; the precise minute each patch shipped should be re-checked on the Steam news hub rather than memorized from this page.\n- The What changed in 1.0.3 callout lists every bug Patch 1.0.3 closes; the Known issues section lists only the bugs that remain open after 1.0.3. Readers should treat both lists as a snapshot, not a final inventory.\n- Console, mobile, and demo versions are not announced as of 2026-09-27; do not promote them to confirmed status.\n- Pricing and the introductory-offer window are tracked on the [price page](/price); this page does not duplicate that figure."
  },
  {
    "id": "faq",
    "type": "prose",
